...
const tiktokdl = async (m, sock) => {
  ...
  if (cmd === "tiktokdl" || cmd === "tiktok") {
    if (!q) return reply(`✨ You should provide a tiktok link. Example: ${prefix}${cmd} https://vm.tiktok.com/xxxx/ ✨`);
    if (!q.includes("tiktok.com")) return reply("⚠️ That doesn't look like a valid TikTok link.");

    await reply("⏳ *Connecting to TikTok Servers...*\n📡 Fetching your video, please hold... Initiating download 📥... Please be patient! ⏳");
