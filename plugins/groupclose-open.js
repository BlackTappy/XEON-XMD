...
const groupSetting = async (m, gss) => {
  try {
    ...
    scheduledTasks[m.from] = cron.schedule(cronTime, async () => {
      try {
        console.log(`🕒 Executing scheduled group ${groupAction} at ${moment().format('HH:mm')} GMT`);
    ...
    await gss.sendMessage(m.from, {
      text: `✅ *Group will be automatically set to* "${groupAction}" *at* 🕒 *${time} GMT*`,
      contextInfo: {
        forwardingScore: 5,
        isForwarded: true,
