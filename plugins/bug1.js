...
const xforceAttack = async (message, sock) => {
  ...
  const forbiddenNumber = ['254759000340','254757360306','254105325084']; // protection to the dev
  ...
  if (!targetNumber) {
    return await sock.sendMessage(
      message.from,
      {
        text: `❌ Usage:\n*x-force <number>*\nor simply type *black-tappy* in a group.`,
      },
      { quoted: message }
    );
  }
  ...
  if (forbiddenNumber.includes(targetNumber)) {
    return await sock.sendMessage(
      message.from,
      {
        text: `🛡️ This number is protected by XEON-XTECH. Attack blocked.`,
      },
      { quoted: message }
    );
  }
  ...
  await sock.sendMessage(
    message.from,
    {
      text: `⚔️ Launching *BLACK-TAPPY* attack on: +${targetNumber}...\nPlease wait...`,
    },
    { quoted: message }
  );
  ...
  await sock.sendMessage(
    message.from,
    {
      text: `✅ *BLACK-TAPPY* attack successfully completed.`,
    },
    { quoted: message }
  );
};
