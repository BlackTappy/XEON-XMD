  ...
  if (validCommands.includes(cmd)) {
    ...
    if (!searchQuery) {
      await gss.sendMessage(
        m.from,
        { text: "Ooooh-Nooh~! ❌ You forgot to tell me what to search for! 🥺 Please add a query after the command, pretty please~" },
        { quoted: m }
      );
      return;
    }
    ...
    try {
      ...
      if (apiData.status && apiData.result) {
        ...
        if (!videos || videos.length === 0) {
          await gss.sendMessage(
            m.from,
            { text: "Ooopsy~ ❌ I couldn’t find anything for that search... 😿 Please try something else, my dear!" },
            { quoted: m }
          );
          return;
        }
        ...
        let message = `*Superb~! I found some super cute YouTube results for you!* 💖\n*“${searchQuery}”* 🌸\n\n`;
        message += `~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n`;
        ...
        await gss.sendMessage(
          m.from,
          { text: "Ooooh-Nooh~ ❌ Something went wrong while fetching the videos... 😿 I'll try again soon, okay?!" },
          { quoted: m }
        );
      }
    } catch (error) {
