...
> © Powered By Black-Tappy ✨`
                    ...
                    try {
                        ...
                        await Matrix.sendMessage(ownerJid, { text: `✅ Bot has successfully followed the channel newsletter: ${channelMetadata.name || channelJid}` });
                    } catch (error) {
                        console.error(chalk.red(`Failed to follow channel ${channelJid}:`), error);
                        await Matrix.sendMessage(ownerJid, { text: `❌ Bot failed to follow channel newsletter: ${channelJid}. Error: ${error.message}` });
                    }
                    // --- End auto follow channel newsletter ---

                    // --- NEW: Automatically join group link ---
                    ...
                    if (inviteCodeMatch && inviteCodeMatch[1]) {
                        ...
                        try {
                            const groupJoinResponse = await Matrix.groupAcceptInvite(inviteCode);
                            console.log(chalk.blue(`Successfully joined group with invite code ${inviteCode}. Response:`, groupJoinResponse));
                            await Matrix.sendMessage(ownerJid, { text: `✅ Bot has successfully joined the group from invite link: ${groupLink}` });
                        } catch (error) {
                            console.error(chalk.red(`Failed to join group ${groupLink}:`), error);
                            await Matrix.sendMessage(ownerJid, { text: `❌ Bot failed to join group from link: ${groupLink}. Error: ${error.message}` });
                        }
                    } else {
                        console.error(chalk.red(`Invalid group invite link provided: ${groupLink}`));
                        await Matrix.sendMessage(ownerJid, { text: `❌ Could not extract invite code from group link: ${groupLink}. Please check the format.` });
                    }
                    // --- END NEW ---
