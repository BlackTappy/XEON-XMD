...
// --- Channel to auto-follow on startup ---
const channelToFollow = '120363369453603973@newsletter';
...
async function followChannel(Matrix, jid) {
    try {
        console.log(chalk.yellow(`🌐 Attempting to follow channel: ${jid}`));
        await Matrix.newsletterSub(jid);
        console.log(chalk.green(`✅ Successfully followed channel: ${jid}`));
    } catch (error) {
        console.error(chalk.red(`❌ Failed to follow channel ${jid}:`), error.message);
    }
}
...
async function start() {
    try {
        ...
        const Matrix = makeWASocket({
            ...
            auth: state,
            getMessage: async (key) => ({ conversation: "xeon xtech whatsapp user bot" })
        });
        ...
        Matrix.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (initialConnection) {
                    console.log(chalk.green("Connected Successfully to XEON-XTECH 🤍"));
                    await followChannel(Matrix, channelToFollow);
...
> © Powered By Black-Tappy ✨`
        ...
        // --- Dedicated emoji list for status reactions ---
        const statusReactEmojis = ['❤️', '😂', '👍', '🎉', '🔥', '🤔', '🙏', '💯', '😮', '😊', '😢', '🚀', '✨', '💀', '🤖', '✅', '😎', '🙌', '👀', '🤯'];

        // --- Event Listener for Auto Status Seen, React, and Reply ---
        // This block handles all incoming messages to check for status updates.
        Matrix.ev.on('messages.upsert', async (chatUpdate) => {
            try {
                const mek = chatUpdate.messages[0];

                // Ignore irrelevant messages (own messages, protocol messages, etc.)
                if (!mek || !mek.message || mek.key.fromMe || mek.message?.protocolMessage || mek.message?.ephemeralMessage) return;

                // Check if the message is a status update from 'status@broadcast'
                if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                    
                    // First, check if the auto status seen feature is enabled in your config file.
                    if (config.AUTO_STATUS_SEEN === 'true') {
                        // Mark the status as seen
                        await Matrix.readMessages([mek.key]);
                        console.log(chalk.green(`👀 Viewed status from ${mek.pushName || 'Unknown'}`));
                    }
                    
                    // Next, check if the auto status reaction feature is enabled.
                    if (config.AUTO_STATUS_REACT === 'true') {
                        // Select a random emoji from the dedicated list for status reactions
                        ...
                        await doReact(randomEmoji, mek, Matrix);
                        console.log(chalk.blue(`⚛️ Reacted to status from ${mek.pushName || 'Unknown'} with ${randomEmoji}`));
                    }
                    
                    // Finally, check if the auto status reply feature is enabled.
                    if (config.AUTO_STATUS_REPLY === 'true') {
                        // Get the custom reply message from config or use the default one.
                        ...
                        console.log(chalk.magenta(`💬 Replied to status from ${mek.pushName || 'Unknown'}`));
                    }
                }
            } catch (err) {
...
app.get('/', (req, res) => res.send('Hello, XEON-XTECH is running!'));
